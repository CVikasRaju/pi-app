...
const { handleVoiceRequest } = require('./handlers/voice');
const { handleGraphRequest } = require('./handlers/graph');
const { handleMLRequest }    = require('./handlers/ml');
const { handleAnalyticsRequest } = require('./handlers/analytics');
const { handleTraceRequest } = require('./handlers/trace');
const { handleLockdownRequest } = require('./handlers/lockdown');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (segments[0] === 'api' && segments[1] === 'chat') {
      ...
      if (sub === 'trace') {
        // GET /api/chat/trace?turnId=
        if (method !== 'GET') return send405(res);
        return await handleTraceRequest(catalystApp, req, res);
      }
    ...
    // ── Phase 2: /api/voice/* ──────────────────────────────────────────────
    if (segments[0] === 'api' && segments[1] === 'voice') {
      const sub = segments[2]; // 'stt' or 'tts'
      return await handleVoiceRequest(catalystApp, req, res, sub);
    }

    // ── Phase 3: /api/graph/* ──────────────────────────────────────────────────
    if (segments[0] === 'api' && segments[1] === 'graph') {
      return await handleGraphRequest(catalystApp, req, res);
    }

    // ── Phase 4: /api/ml/* ────────────────────────────────────────────────────
    if (segments[0] === 'api' && segments[1] === 'ml') {
      return await handleMLRequest(catalystApp, req, res, segments);
    }

    // ── Phase 4: /api/analytics/* ────────────────────────────────────────────
    if (segments[0] === 'api' && segments[1] === 'analytics') {
      return await handleAnalyticsRequest(catalystApp, req, res, segments);
    }

    // ── Phase 5: /api/audit/* ──────────────────────────────────────────────
    if (segments[0] === 'api' && segments[1] === 'audit') {
      const sub = segments[2]; // 'lockdown-report' | 'log'
      if (method !== 'GET') return send405(res);
      return await handleLockdownRequest(catalystApp, req, res, sub);
    }
